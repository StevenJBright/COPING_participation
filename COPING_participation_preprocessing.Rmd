...
GLAD_signup_age_filter$dem.which_gender_do_you_identify_with_labelled <- factor(
...
#Check distribution
GLAD_signup_age_filter %>% 
  freq(dem.which_gender_do_you_identify_with_labelled)
  ...
  mutate(highest_education = 
           case_when(
             ...
             (dem.college_or_university_degree == 0 &
                ...
                dem.nvq_or_hnd_or_hnc_or_equivalent == 0 ~ 6)
           ))
...
GLAD_signup_gender_recode$highest_education_labelled <- factor(
  ...
  labels = c("College or university degree", "A levels/AS levels or equivalent",
             ...
             "NVQ or HND or HNC or equivalent", "None of the above")
  )
...
glad_signup_total_waves <- glad_signup_total_waves %>% 
  mutate(total_COPING_waves_3levels = 
           case_when(
         total_COPING_waves == 0 ~ 0,
         total_COPING_waves == 1 ~ 1,
         total_COPING_waves > 1 ~ 2
           )
         )

#Check output
glad_signup_total_waves %>% 
  group_by(total_COPING_waves_3levels) %>% 
  count()
...
glad_signup_total_waves$total_COPING_waves_3levels_labelled <- factor(
  glad_signup_total_waves$total_COPING_waves_3levels,
  levels = c(0, 1, 2),
  labels = c("0 surveys", "1 survey", "2+ surveys")
  )

#Check output
glad_signup_total_waves %>% 
  group_by(total_COPING_waves_3levels_labelled) %>% 
  count()
