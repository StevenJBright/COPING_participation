...
GLAD_signup_gender_recode$gender_recode_labelled <- factor(
  GLAD_signup_gender_recode$gender_recode,
  levels = c(0, 1, 2),
...
```{r Combine binary education variables into single variable}
...
GLAD_signup_gender_recode <- GLAD_signup_gender_recode %>% 
  mutate(highest_education = 
           case_when(
             ...
             dem.cses_or_equivalent == 1 ~ 3,
             dem.nvq_or_hnd_or_hnc_or_equivalent == 1 ~ 4,
             (dem.college_or_university_degree == 0 &
                dem.a_levelsas_levels_or_equivalent == 0 &
                dem.o_levelsgcses_or_equivalent == 0 &
                dem.cses_or_equivalent == 0 &
                dem.nvq_or_hnd_or_hnc_or_equivalent == 0 ~ 5)
           ))

```

```{r Label the highest education variable}

GLAD_signup_gender_recode$highest_education_labelled <- factor(
  GLAD_signup_gender_recode$highest_education,
  levels = c(1, 2, 3, 4, 5),
  labels = c("College or university degree", 
             "A levels/AS levels or equivalent",
             "O levels/GCSEs or CSEs or equivalent",
...
#Check output
GLAD_signup_gender_recode %>% 
  group_by(highest_education_labelled) %>% 
  count()
...
```{r Create labelled factor variable for recoded partnership status}

GLAD_signup_gender_recode$partnership_status_labelled <- factor(
  GLAD_signup_gender_recode$partnership_status,
  levels = c(1, 2, 3),
  labels = c("Single", "Relationship or married/civil partnership", "Divorced/widowed/separated")
)

#Check distribution
GLAD_signup_gender_recode %>% 
  freq(partnership_status_labelled)
...
```{r Create age groups variable}

GLAD_signup_cancer_recode <- GLAD_signup_cancer_recode %>% 
  mutate(age_groups = 
           case_when(
             age_cleaned >= 16 & age_cleaned <= 18 ~ 1,
             age_cleaned >= 19 & age_cleaned <= 25 ~ 2,
             age_cleaned >= 26 & age_cleaned <= 35 ~ 3,
             age_cleaned >= 36 & age_cleaned <= 45 ~ 4,
             age_cleaned >= 46 & age_cleaned <= 55 ~ 5,
             age_cleaned >= 56 & age_cleaned <= 65 ~ 6,
             age_cleaned >= 66 & age_cleaned <= 70 ~ 7,
             age_cleaned >= 71 & age_cleaned <= 75 ~ 8,
             age_cleaned >= 76 ~ 9
           ))
...
```{r Created a labelled factor of age groups}

GLAD_signup_cancer_recode$age_groups_labelled <- factor(
  GLAD_signup_cancer_recode$age_groups,
  levels = c(1, 2, 3, 4, 5, 6, 7, 8, 9),
  labels = c("16-18", "19-25", "26-35", "36-45", "46-55", "56-65", "66-70",
             "71-75", "76+")
  )

#Check output
GLAD_signup_cancer_recode %>% 
  group_by(age_groups_labelled) %>% 
  count()
...
GLAD_signup_first_invitation$joined_coping_first_invitation_labelled <- factor(
...
GLAD_signup_first_invitation %>% 
  group_by(joined_coping_first_invitation_labelled) %>% 
  count()
...
GLAD_signup_depression_and_anxiety$depressive_and_anxiety_comorbidity_labelled <- factor(
  ...
  levels = c(1, 0, 2, 3),
  labels = c("Depressive and anxiety disorder", " No depressive or anxiety disorder",
...
glad_signup_psychosis_and_bipolar$asd_labelled <- factor(
...
glad_signup_MH_comorbidity$mental_health_comorbidities_labelled <- factor(
  ...
  levels = c(0, 1, 2),
  labels = c("No mental health disorders", "1 mental health disorder",
...
glad_signup_phy_comorbidities <- glad_signup_total_phy_disorders %>% 
  mutate(physical_health_comorbidities =
           case_when(
             phy_disorders_total_count == 0 ~ 0,
             phy_disorders_total_count == 1 ~ 1,
             phy_disorders_total_count > 1 ~ 2
           ))
...
glad_signup_phy_comorbidities$physical_health_comorbidities_labelled <- factor(
 ...
 levels = c(0, 1, 2),
 labels = c("No physical health disorders", "1 physical health disorder",
...
glad_signup_completed_baseline <- glad_signup_phy_comorbidities %>% 
...
#Calculate every pt's total completed follow-up surveys with row sums
...
#Ensure that pt's who didn't complete the COPING baseline survey get NA
glad_signup_total_waves <- glad_signup_total_waves %>% 
  mutate(total_COPING_waves = 
           case_when(
             
             #People who didn't complete baseline survey didn't receive follow-up surveys
             completed_baseline == 0 ~ NA_real_,
...
```{r Create labelled version of total COPING waves}
...
For plots and tables, we will create a collapsed COPING surveys variable.

```{r Create a 0 1 or 2+ COPING surveys completed variable}
...
```{r Add labelled version of total_COPING_waves_3levels}
...
-Pt's with NA for gender
-Pt's with NA/implausible value for age
...
glad_signup_final <- glad_signup_total_waves %>% 
  filter(age_cleaned >= 16 &
           age_cleaned <= 117) %>%
  filter(!is.na(gender_recode_labelled)) %>%
  filter(joined_coping_first_invitation_labelled != "Not applicable")
