...
coping_followups_outcome <- c("total_COPING_surveys")
...
#Main aim 2 model formula
aim2_ZINB_model_explan_vars <- explanatory_variables %>% 
  str_subset(pattern = "startDate",
             negate = TRUE)

aim2_followup_participation_formula <- as.formula(paste(coping_followups_outcome, 
                                                        paste(aim2_ZINB_model_explan_vars, 
                                                              collapse=" + "), 
                                                        sep = " ~ "))
...
###Only participants who have provided a saliva kit
17/07/2022
...
#Filter the participants who have returned a saliva kit
glad_coping_participation_returned_kit <- glad_coping_participation %>% 
  filter(kit_returned == 1)

#Investigate factors associated with COPING baseline survey participation in this cohort
baseline_participation_kit_returners_LR_model <- glm(
  formula = completed_baseline_labelled ~ age_cleaned + 
    gender_recode_labelled +
    highest_education_labelled +
    ethnicity_labelled +
    ...
    ADHD_ADD_labelled +
    personality_disorder_labelled +
    PTSD_labelled + #Why is estimate NA?
    startDate,
  family = "binomial",
  data = glad_coping_participation_returned_kit)

#Check output
summary(baseline_participation_kit_returners_LR_model)
```

```{r Create vector of explanatory variables in providing a kit LR model}
baseline_participation_kit_returners_LR_model_names <- c(
  "Intercept",
  "age_cleaned",
  "gender_recode_labelledFemale",
  "gender_recode_labelledNon-binary/prefer to self-define",
...
#Create tidy model output
baseline_participation_kit_returners_LR_model_tidy <- tidy(
  baseline_participation_kit_returners_LR_model,
  exponentiate = T,
  conf.int = T) %>%
  mutate(
    Variable = baseline_participation_kit_returners_LR_model_names,
    Significance = if_else(
      p.value < 0.05, 
      1,
      0
...
#Check output
baseline_participation_kit_returners_LR_model_tidy
```
...
```{r Put ZINB explanatory variavles into a vector}
ZINB_kit_returners_variables <- c(
  "age_cleaned",  
  "gender_recode_labelled", 
  "highest_education_labelled",
  "ethnicity_labelled",
  ...
  "ADHD_ADD_labelled",
  "personality_disorder_labelled"
  #"PTSD_labelled" # Including PTSD throws an error? - Investigate
  #"kit_returned_labelled" # Should we leave this out?
  )
```

```{r Run ZINB model}
ZINB_kit_returners_model <- zeroinf.neg.binomial.estimates(
  outcome = "total_COPING_waves",
  explanatory_variables = ZINB_kit_returners_variables,
  data_set = "glad_coping_participation_returned_kit",
  name = "ZINB_kit_returners_model"
)

#Check ZINB model output
ZINB_kit_returners_model %>% 
  head()
```

```{r Write ZINB_kit_returners_model results to csv file}
ZINB_kit_returners_model %>% 
  write_csv("../ZIMB_kit_returners_participation_results.csv")
```
