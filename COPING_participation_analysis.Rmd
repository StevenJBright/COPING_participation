...
age_hist <- continuous_vars_subset %>% 
...
phq_hist <- continuous_vars_subset %>% 
  ggplot(aes(x = phq.sum_score)) +
  geom_histogram(bins = 20, colour = "Black", fill = "#EFC10B") +
  theme_minimal() +
  theme(axis.text.x = element_text(colour = "black"),
        axis.text.y = element_text(colour = "black")) +
...
gad_hist <- continuous_vars_subset %>% 
  ggplot(aes(x = gad.sum_score)) +
  geom_histogram(bins = 20, colour = "Black", fill = "#EFC10B") +
  theme_minimal() +
  theme(axis.text.x = element_text(colour = "black"),
        axis.text.y = element_text(colour = "black")) +
...
```{r Check for outliers in GAD anxiety scores}
audit_hist <- continuous_vars_subset %>% 
  ggplot(aes(x = audit.sum_score)) +
  geom_histogram(bins = 20, colour = "Black", fill = "#EFC10B") +
  theme_minimal() +
  theme(axis.text.x = element_text(colour = "black"),
        axis.text.y = element_text(colour = "black")) +
...
MH_disorder_count_hist <- continuous_vars_subset %>% 
...
MH_disorder_count_hist

# Save plot
ggsave("../MH_disorders_count_hist.jpeg",
       plot = MH_disorder_count_hist,
       width = 10,
       height = 6,
       dpi = 150,
       units = "in")
...
Since the Box-Tidwell function doesn't work with variables that have negative numbers or 0, we can only test the age variable using the Box-Tidwell function. For the remaining variables, we can plot its relationship with the logit transformation of the outcome variable. 

```{r Perform Box-Tidwell test for age variable}
