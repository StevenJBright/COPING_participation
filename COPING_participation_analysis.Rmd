...
sociodemographic_LR_variable_names <- c(
  "Intercept",
  "age_cleaned",
...
sociodemographic_LR_model_tidy <- tidy(
  ...
  conf.int = T) %>%
  mutate(
    Variable = sociodemographic_LR_variable_names,
...
psychiatric_and_health_LR_model <- glm(
  formula = completed_baseline_labelled ~ phq.sum_score + 
    gad.sum_score +
    audit.sum_score +
...
psychiatric_and_health_LR_variable_names <- c(
  "Intercept",
  "phq.sum_score",
  "gad.sum_score",
  "audit.sum_score",
  ...
  "mental_health_comorbidities_labelled1 mental health disorder",
  "mental_health_comorbidities_labelled2+ mental health disorders",
  ...
  "autism_spectrum_disorderAutistic spectrum disorder",
...
glad_study_LR_variable_names <- c(
  "Intercept",
  "startDate",
  "kit_returnedPt has provided a saliva kit",
...
LR regression assumptions listed here: 
...
```{r Create cooks distance plot function}
...
```{r Check for outliers in sociodemographic model}
...
```{r Check for outliers in psychiatric and health model}
...
```{r Check for outliers in glad study variable model}
...
This is an additional check for multicollinearity.

###Sociodemographic correlation matrix
...
###glad study variables

```{r Create correlation matrix for glad study variables}

#Put GLAD study variables into a vector
...
glad_study_variables_correlation_vars <- glad_coping_participation_raw %>% 
  select(all_of(glad_study_variable_names[-1])) #Remove startDate
...
```{r Create correlation plot for glad study variables}
