...
##Get confidence intervals for ZINB model

This stackoverflow page walks through how you can get CIs for a ZINB model
via boostrapping:
https://stackoverflow.com/questions/58002334/how-to-add-confidence-intervals-for-a-zero-inflated-negative-binomial-function-i

```{r Get predicted ZINB values}

#ZINB_predict <- ZINB_model %>% 
#  predict()
...
```{r Define stat function}

#stat <- function(df, inds) {
#    model <- formula(total_COPING_waves ~ age_cleaned + 
#    gender_recode_labelled +
#    highest_education_labelled +
#    ethnicity_labelled +
#    employment_labelled +
...
```{r Run bootstrapping with stat function}

#set.seed(2018)
#res <- boot(glad_coping_participation_raw, 
#            stat, 
#            R = 100)
...
#  as.data.frame(
#    t(
#      sapply(1:nrow(glad_coping_participation_raw), 
#             function(row)
#               boot.ci(res, 
#                       conf = 0.95, 
#                       type = "basic", 
