...
```{r Create age plot for COPING surveys completion}

age_group_COPING_waves_plot <- glad_coping_participation %>% 
  ggplot(aes(x = age_groups_labelled, fill = total_COPING_waves_3levels_labelled)) +
  geom_bar(position = "dodge") +
  labs(x = "Age (years)",
       y = "N participants") +
...
glad_coping_participation %>% 
  select(all_of(ends_with("_labelled")),
         ...
         -total_COPING_waves_3levels_labelled) %>% 
...
glad_coping_participation %>% 
  select(all_of(ends_with("_labelled")),
         ...
         -total_COPING_waves_labelled) %>% 
  tbl_summary(missing_text = "Missing",
              by = "total_COPING_waves_3levels_labelled")
